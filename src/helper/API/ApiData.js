...
export const ApiGet = (type) => {
    ...
    return new Promise((resolve, reject) => {
        axios.get(`${BaseURL}${type}${s}`, getHttpOptions())
            .then((responseJson) => {
...
export const ApiGetNoAuth = (type) => {
    const s = type.includes('?') ? '&' : '?';
    return new Promise((resolve, reject) => {
        axios.get(`${BaseURL}${type}${s}`, getHttpOptions({ ...defaultHeaders, isAuth: false }))
            .then((responseJson) => {
                resolve(responseJson.data);
            })
            .catch((error) => {
...
export const ApiPatchNoAuth = (type, userData) => {
    const s = type.includes('?') ? '&' : '?';
    return new Promise((resolve, reject) => {
        axios.patch(`${BaseURL}${type}${s}`, userData, getHttpOptions({ ...defaultHeaders, isAuth: false }))
            .then((responseJson) => {
                resolve(responseJson.data);
            })
            .catch((error) => {
...
export const ApiDelete = (type, userData) => {
    const s = type.includes('?') ? '&' : '?';
    return new Promise((resolve, reject) => {
        axios.delete(`${BaseURL}${type}${s}`, getHttpOptions())
            .then((responseJson) => {
                resolve(responseJson);
            })
            .catch((error) => {
